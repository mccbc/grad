...
cluster_data = cluster_data[np.logical_and(cluster_masses >= 1e14, cluster_distances <= 225)]
cluster_masses = cluster_masses[np.logical_and(cluster_masses >= 1e14, cluster_distances <= 225)]
...
for i in range(1, n_bins+1):
    ...
    #    rule = dist_inds == k
        ...
        if len(elliptical_flags[rule]) > 10:
            frac_ellip.append(sum(elliptical_flags[rule])/len(elliptical_flags[rule]))
            bincenters.append(all_bincenters[k-1])

    # Plot results
#    ax.step(bincenters, frac_ellip, where='mid', alpha=0.75, c=colors[i-1], label='{} $< M_{{200}} < $ {}'.format(scinot(mass_bins[i-1]), scinot(mass_bins[i])))
    ax.plot(bincenters, frac_ellip, alpha=0.75, c=colors[i-1], lw=1, marker='o', markersize=2, label=r'{} $< \rm M_{{200}} < $ {} $\rm M_{{\odot}}$'.format(scinot(mass_bins[i-1]*1e14), scinot(mass_bins[i]*1e14)))


if n_bins > 1:
    sm = plt.cm.ScalarMappable(cmap=pl.cm.jet, norm=plt.Normalize(vmin=min(cluster_masses*1e14), vmax=max(cluster_masses*1e14)))
